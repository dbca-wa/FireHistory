...
on connection speeds. Make sure you save it somewher locally on your computer and 
...
### DBCA tenure AOI
Want to use a National Park boundary as the AOI but don't know how to get it it 
what it's full name is? No problems. The package ships with a complete listing 
of tenure names as well as their boundaries. 
```{r, message=FALSE, warning=FALSE}
# have a look at the first few records....there's 1000+
head(find_tenure())
```
...
Now that we have the correct name we can retrieve the tenure boundary and save it 
as our AOI.
```{r, eval=FALSE}
...
fire history is best on DBCA tenure and generally even better in more populous 
regions/areas.

## Fire Metrics
Now it is time to generate some fire metrics. Here is an example to calculate 
Year Since Last Burn (YSLB).
```{r, eval=FALSE}
